...
class OrderGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Order entity")
        ...
        .repeat(2) {
            ...
            .get("/mallserviceorder/api/orders")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new order")
            .post("/mallserviceorder/api/orders")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "code":"SAMPLE_TEXT", "totalAmount":null, "timeCreated":"2020-01-01T00:00:00.000Z", "state":null}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_order_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created order")
                .get("/mallserviceorder${new_order_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
